...
- **玩法重构（禅跳 × 放下越轻越高，gray-box 已跑通）**：核心变量「轻盈度」——正心落点（正中甜区）
  卸下负累 → 变轻 → 有效重力更小 → 越跳越高越飘；柔性下沉不硬重开；负累化光 + 放下短诗 +
  生成式落点音效；正念踏石 / 上升气流两种机关。设计见 `docs/gameplay-zen-lightness.md`。
  等 Claude Design 出精致 UI 后做"上皮"版。
